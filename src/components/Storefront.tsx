...
import { formatCep } from "@/core/delivery";
...
function Icon({
  ...
  name: "bag" | "chat" | "close" | "plus" | "minus" | "arrow";
}) {
  const paths = {
    bag: (
      <>
...
export function Storefront({ tenant }: Props) {
  ...
  const [category, setCategory] = useState<
    "todos" | "brownies" | "trufas"
  >("todos");
  const [orderMode, setOrderMode] = useState<"Entrega" | "Retirada">(
    "Entrega",
  );
  ...
  >([
    {
      from: "bot",
      text: "Oi! Posso ajudar com cardápio, entrega, retirada e horário. O atendimento só abre quando você quiser.",
    },
  ]);
  ...
  const deliveryFee =
    orderMode === "Entrega" ? (deliveryQuote?.fee ?? 0) : 0;
  ...
  function checkout() {
    ...
    const lines = cartProducts.map(
      ({ line, product }) =>
        `• ${line.quantity}x ${product.name} — ${money.format(
    ...
    const text = [
      ...
      `Meu nome é ${name.trim()} e quero fazer este pedido:`,
      "",
      ...lines,
      "",
      ...
      orderMode === "Entrega" && deliveryQuote
        ...
        : "",
      orderMode === "Entrega" && deliveryQuote
        ? `Taxa de entrega: ${money.format(deliveryQuote.fee)}`
        : "",
      `Total: ${money.format(orderTotal)}`,
      "",
      orderMode === "Entrega"
        ? "Aguardo a confirmação do endereço, disponibilidade e prazo."
        : "Aguardo a confirmação do horário para retirada.",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    <>
      ...
      <main>
        ...
        >
          ...
          <div className="hero-content">
            <span className="eyebrow">
              Feito em Blumenau • {tenant.operation.hours}
            </span>
            <h1>
              Seu momento pede <em>chocolate de verdade.</em>
            </h1>
            <p>{tenant.brand.description}</p>
            <div className="hero-buttons">
              <a className="button primary" href="#cardapio">
                Escolher agora <Icon name="arrow" />
              </a>
              <a
                className="button ghost"
                href={tenant.contact.ifoodUrl}
                target="_blank"
                rel="noreferrer"
              >
                Pedir no iFood
            ...
            <div className="proof-row">
              <span>{tenant.operation.delivery}</span>
        ...
        <section className="manifesto">
          <span>Casquinha delicada.</span>
          <span>Centro úmido.</span>
          <span>Vontade resolvida.</span>
        </section>
        ...
        <section className="feature">
          <div className="feature-photo">
            <Image
              src="/tenants/melt/brownie.webp"
              alt="Brownie Melt com casquinha fina"
              fill
              sizes="50vw"
            />
          </div>
        ...
        <section id="entrega" className="delivery">
      ...
      <footer>
        ...
        <div>
          <b>Pedidos</b>
          <a href="#cardapio">Cardápio</a>
          <a href={tenant.contact.ifoodUrl}>iFood</a>
        </div>
        <div>
          <b>Contato</b>
          <a href={tenant.contact.instagramUrl}>
            {tenant.contact.instagramLabel}
          </a>
          <span>{tenant.operation.deliveryPricing.origin.address}</span>
        </div>
      </footer>

      <div
        className={`backdrop ${drawer ? "visible" : ""}`}
        onClick={() => setDrawer(null)}
      />

      <aside
        className={`drawer ${drawer === "cart" ? "open" : ""}`}
      ...
      >
        ...
        <div className="drawer-footer">
          <div className="total">
            <span>Produtos</span>
            <b>{money.format(productsTotal)}</b>
          </div>
      ...
      <aside
        className={`drawer assistant-drawer ${
          drawer === "assistant" ? "open" : ""
        }`}
        aria-hidden={drawer !== "assistant"}
      >
        <div className="drawer-head">
          <div>
            <span className="eyebrow dark">
